...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.labelStats = wx.StaticText(self.panel, -1, u"Estadísticas módulos actuales")
    ...
    def __set_properties(self):
        ...
        ciclo_actual = datetime.date.today()
        self.ciclo_actual = ciclo_actual.year
        c.execute('''SELECT id_curso, num_curso FROM cursos WHERE ciclo = %s;''', (self.ciclo_actual))
        ...
        for curso in cursos_totales:
            ...
            for alumno in alumnos_en_curso:
                ...
                cadalum = c.fetchone()
                listado_inicial.append(cadalum[1])
                if cadalum[0] == 'F':
                    chicas_tot += 1
                else:
                    chicos_tot += 1
        ...
        c.execute('''SELECT id_curso, num_curso FROM cursos WHERE ciclo = %s and estado = %s;''',
         (self.ciclo_actual, u'A'))
        cursos_activos = c.fetchall()
        chicas_tot = chicos_tot  = 0
        listado_inicial = []
        for curso in cursos_activos:
            ...
            for alumno in alumnos_en_curso:
                ...
                if cadalum[0] == 'F':
                    ...
                    chicos_tot += 1
        alumnos_este_ciclo = len(listado_inicial)
        lista = [u'Estudiantes al iniciar los cursos activos', chicas_tot, chicos_tot, alumnos_este_ciclo]
        index = self.listStats.InsertStringItem(sys.maxint, str(lista[0]))
        for col, text in enumerate(lista[1:]):
            self.listStats.SetStringItem(index, col+1, str(text))
        
        # Los que realmente están cursando
        chicas = 0
        chicos = 0
        listado_general = []
        for curso in cursos_activos:
            ...
            for alumno in alumnos_en_curso:
                c.execute('''SELECT sexo, id_alumno FROM alumnos WHERE id_alumno = %s''' % alumno[0])
                cadalum = c.fetchone()
                listado_general.append(cadalum[1])
                if cadalum[0] == 'F':
                    chicas += 1
                else:
                    chicos += 1
        alumnos_este_ciclo = len(listado_general)
        if alumnos_este_ciclo == 0:
            return
        lista = [u'Estudiantes cursando actualmente', chicas, chicos, alumnos_este_ciclo]
        index = self.listStats.InsertStringItem(sys.maxint, str(lista[0]))
        for col, text in enumerate(lista[1:]):
            self.listStats.SetStringItem(index, col+1, str(text))
        ...
        for alumno in listado_general:
            ...
            anio = cadalum[1]
            anio = anio.year
            if 18 < self.ciclo_actual - anio < 31:
                if cadalum[0] == 'F':
                    chicas_jov += 1
                else:
                    chicos_jov += 1
